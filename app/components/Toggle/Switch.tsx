...
function SwitchInput(props: SwitchInputProps) {
  ...
  const offBackgroundColor = [
    disabled && colors.palette.yellow.s100,
    status === "error" && colors.errorBackground,
    colors.palette.yellow.s100,
  ].filter(Boolean)[0]
...
function SwitchAccessibilityLabel(props: SwitchInputProps & { role: "on" | "off" }) {
  ...
  const color = (function () {
    if (disabled) return colors.palette.yellow.s100
    if (status === "error") return colors.error
    if (!on) return innerStyle?.backgroundColor || colors.palette.yellow.s100
    return detailStyle?.backgroundColor || colors.palette.yellow.s100
  })()
